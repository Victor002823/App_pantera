...
function enviarAPantera() {
    ...
    const btn = document.getElementById("btnEnviarPantera");
    const icon = document.getElementById("btnEnviarPanteraIcon");
    ...
    .then(data => {
        ...
        return fetch("https://app-pantera.onrender.com/view/home/recibir_transferencia.php", {
            method: "POST",
    ...
    .then(resp => {
        if (resp.success) {
            CustomAlert.show({ title: "Enviado", text: "Cotizacion enviada a Pantera correctamente (ID nuevo: " + resp.nuevo_id + ").", icon: "success" });
        } else {
            CustomAlert.show({ title: "Error", text: resp.error || "No se pudo enviar a Pantera.", icon: "error" });
        }
    })
    .catch((err) => {
        alert("DEBUG error: " + err.message);
        CustomAlert.show({ title: "Error", text: "Error de red al enviar a Pantera.", icon: "error" });
    })
    .finally(() => {
        btn.disabled = false;
